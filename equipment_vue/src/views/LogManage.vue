...
<script>
    export default {
        ...
        methods:{
            page(currentPage){
                const _this = this
                _this.axios.get('/logs/list?page='+currentPage+'&size='+_this.pageSize).then(function (response) {
        ...
        created() {
            const _this = this
            _this.axios.get('/logs/list?page=1&size='+_this.pageSize).then(function (response) {
                _this.tableData = response.data.data
                _this.pageSize = response.data.size
                _this.total = response.data.total
            })
